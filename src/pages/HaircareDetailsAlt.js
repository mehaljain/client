...
export default function HaircareDetailsAlt() {
  const { id } = useParams();
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      setError(null);
      try {
        const url = `${API_BASE}/api/haircare/${encodeURIComponent(id)}?t=${Date.now()}`;
        ...
        if (!incoming || Object.keys(incoming).length === 0) {
          setProduct(null);
          setError("Product not found or empty response from server.");
        } else {
        ...
        console.error("[HaircareDetailsAlt] fetch error:", err);
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          ...
          <Link to="/haircare" className="inline-block px-6 py-3 bg-green-500 text-white rounded-lg hover:bg-green-600 transition">
            Back to Haircare Products
          </Link>
        </div>
      </div>
    );
  ...
  const hairTypes = (product.hairType || []).filter((h) => h && String(h).trim() !== "");
  const concerns = (product.concern || []).filter((c) => c && String(c).trim() !== "");
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="bg-pink-100 py-8 px-8">
        <div className="max-w-7xl mx-auto">
          <Link to="/haircare" className="inline-flex items-center text-green-600 hover:text-green-700 transition mb-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                {hairTypes.length > 0 && (
                  <div>
                    <h3 className="font-semibold text-gray-800 mb-2">Hair Type:</h3>
                ...
                {concerns.length > 0 && (
                  <div>
                    <h3 className="font-semibold text-gray-800 mb-2">Concerns:</h3>
                    <div className="flex flex-wrap gap-2">
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="text-center bg-white p-6 rounded-lg shadow-md">
              ...
              <p className="text-gray-600">Rigorously tested for safety and effectiveness</p>
            </div>
            <div className="text-center bg-white p-6 rounded-lg shadow-md">
              <div className="text-green-500 mb-4 flex justify-center">
                <svg className="w-12 h-12" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <p className="text-gray-600">Made with premium natural ingredients</p>
            </div>
            <div className="text-center bg-white p-6 rounded-lg shadow-md">
              <div className="text-green-500 mb-4 flex justify-center">
                <svg className="w-12 h-12" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <p className="text-gray-600">Ethically produced and environmentally conscious</p>
            </div>
          </div>
        </div>
      </div>
